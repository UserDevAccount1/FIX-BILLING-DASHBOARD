...
function switchModalTab(tabId) {
    console.log('[Modal Tabs] Switching to tab:', tabId);
    ...
    const allTabButtons = document.querySelectorAll('.modal-tab');
    ...
    if (selectedContent) {
        ...
        console.error(`[Modal Tabs] Tab content not found: ${tabId}-tab-content`);
    }
    
    // Activate the selected tab button
    const selectedTabButton = document.getElementById(`modal-tab-${tabId}`);
    if (selectedTabButton) {
        selectedTabButton.classList.add('active');
    } else {
        console.error(`[Modal Tabs] Tab button not found: modal-tab-${tabId}`);
    }
    
    // Update the tab scroll position if needed
...
function scrollTabIntoView(tabId) {
    const tabButton = document.getElementById(`modal-tab-${tabId}`);
    if (!tabButton) return;
    
    const tabsContainer = document.querySelector('.tabs-scroll-wrapper');
...
function initializeTabContent(tabId) {
    ...
    switch(tabId) {
        case 'dedicated':
            console.log('[Modal Tabs] Initializing Dedicated tab...');
            // Initialize dedicated tab functions if needed
            break;
            
        case 'project':
            console.log('[Modal Tabs] Initializing Project tab...');
            // Initialize project tab functions if needed
            break;
            
        case 'calendar':
            console.log('[Modal Tabs] Initializing Calendar tab...');
            // Initialize calendar tab functions if needed
            break;
            
        case 'analytics':
            console.log('[Modal Tabs] Initializing Analytics tab...');
            // Initialize analytics tab functions if needed
            break;
            
        case 'ai-summary':
            console.log('[Modal Tabs] Initializing AI Summary tab...');
            // Initialize AI summary tab functions if needed
            break;
            
        case 'settings':
            console.log('[Modal Tabs] Initializing Settings tab...');
            // Initialize settings tab functions if needed
            break;
            
        case 'band-version':
            console.log('[Modal Tabs] Initializing Band Version tab...');
            // Initialize band version tab functions if needed
            break;
    }
}
...
function initializeTabEventListeners() {
    console.log('[Modal Tabs] Initializing tab event listeners...');
    ...
    // Also initialize any quick action buttons that switch tabs
    const quickActionButtons = document.querySelectorAll('[onclick^="switchModalTab"]');
    quickActionButtons.forEach(button => {
        // Extract tab ID from onclick attribute
        const onclickAttr = button.getAttribute('onclick');
        const match = onclickAttr.match(/switchModalTab\('([^']+)'\)/);
        if (match && match[1]) {
            const tabId = match[1];
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('[Modal Tabs] DOM loaded, initializing tabs...');
    ...
    setTimeout(() => {
        ...
        if (overviewContent && !overviewContent.classList.contains('active')) {
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(initializeTabEventListeners, 100);
}