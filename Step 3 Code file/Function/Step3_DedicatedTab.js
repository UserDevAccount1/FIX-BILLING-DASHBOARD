...
// Load Sample Dedicated Data with all fields
...
function setupDedicatedEventListeners() {
    ...
    if (customerFilter) {
        customerFilter.addEventListener('change', function() {
            filterDedicatedByCustomer(this.value);
        });
    }
    ...
    if (currencyFilter) {
        currencyFilter.addEventListener('change', function() {
            filterDedicatedByCurrency(this.value);
        });
    }
...
function renderDedicatedTable() {
    ...
    if (filteredDedicatedData.length === 0) {
        tableBody.innerHTML = `
            <tr>
                <td colspan="39" class="dedicated-empty-table-message">
    ...
    filteredDedicatedData.forEach(record => {
        ...
        row.innerHTML = `
            ...
            <td class="dedicated-editable" data-field="workedDays" data-id="${record.id}">
                ${record.workedDays === 0 ? 'NA' : record.workedDays}
            </td>
...
function getDedicatedRowClass(record) {
    if (record.workingDays === 0) return 'dedicated-row-special';
...
function filterDedicatedData(searchTerm) {
    if (!searchTerm || !searchTerm.trim()) {
        ...
        filteredDedicatedData = dedicatedData.filter(record => {
            return (record.technicianName && record.technicianName.toLowerCase().includes(term)) ||
                   (record.country && record.country.toLowerCase().includes(term)) ||
                   (record.band && record.band.includes(term)) ||
                   (record.currency && record.currency.toLowerCase().includes(term)) ||
                   (record.serviceMonth && record.serviceMonth.toLowerCase().includes(term));
        });
    }
    ...
    if (currencyFilter && currencyFilter.value) {
        filteredDedicatedData = filteredDedicatedData.filter(record => 
            record.currency === currencyFilter.value
        );
    }
...
// Filter by Customer
function filterDedicatedByCustomer(customer) {
...
// Filter by Currency
function filterDedicatedByCurrency(currency) {
    const searchInput = document.getElementById('dedicated-search-input');
...
function sortDedicatedData() {
    filteredDedicatedData.sort((a, b) => {
        ...
        switch (currentSortField) {
            case 'technician':
                aValue = a.technicianName ? a.technicianName.toLowerCase() : '';
                bValue = b.technicianName ? b.technicianName.toLowerCase() : '';
                break;
            case 'country':
            ...
            case 'totalCost':
                ...
                break;
            case 'slaPercentage':
                aValue = a.slaPercentage || 0;
                bValue = b.slaPercentage || 0;
                break;
            default:
                aValue = a.technicianName ? a.technicianName.toLowerCase() : '';
                bValue = b.technicianName ? b.technicianName.toLowerCase() : '';
...
function updateDedicatedSummary() {
    ...
    const totalHolidayImpact = filteredDedicatedData.reduce((sum, record) => 
        sum + ((record.holidayDays || 0) * (record.dailyRate || 0)), 0);
...
function populateVersionSelect() {
    ...
    versionSelect._versions = versions;
}
...
function setupVersionModalListeners() {
    ...
    if (versionSelect) {
        versionSelect.addEventListener('change', function() {
            updateVersionDetails(this);
        });
    }
}

// Update Version Details
function updateVersionDetails(select) {
    ...
    // Get versions data
    const versions = select._versions || [];
    const version = versions.find(v => v.id === versionId);
    
    if (!version) return;
    
    // Get preview data (first 3 records from dedicatedData)
    const preview = dedicatedData.slice(0, 3);
...
function confirmFileUpload() {
    ...
    setTimeout(() => {
        ...
        const searchInput = document.getElementById('dedicated-search-input');
        filterDedicatedData(searchInput ? searchInput.value : '');
    }, 2000);
}

// Recalculate Dedicated Data
    ...
    setTimeout(() => {
        dedicatedData.forEach(record => {
            ...
            if (record.workingDays > 0) {
                record.dailyRate = record.monthlyRate / record.workingDays;
                record.actualCost = record.dailyRate * (record.workedDays || 0);
            }
            
            record.convertedMonthlyRate = record.monthlyRate * (currencyRates[record.convertedCurrency] || 1);
            record.convertedDailyRate = record.dailyRate * (currencyRates[record.convertedCurrency] || 1);
            record.convertedActualCost = record.actualCost * (currencyRates[record.convertedCurrency] || 1);
            
            record.adjustedWorkingDays = (record.workingDays || 0) - (record.holidayDays || 0);
            ...
            record.otHoursCost = (record.otHours || 0) * (record.otPerHourRate || 0);
            record.weekendCost = (record.weekendOtHours || 0) * (record.weekendRate || 0);
            
            // Calculate tax cost if tax percentage is applied
            const baseCost = (record.actualCost || 0) + record.otHoursCost + record.weekendCost + (record.travelExtraCost || 0);
            if (record.taxPercentage && record.taxPercentage > 0) {
            ...
            record.totalCostExcel = record.totalCost;
...
function validateDedicatedData() {
    ...
    filteredDedicatedData.forEach(record => {
        ...
        if (record.workedDays > record.workingDays && record.workingDays > 0) {
            warnings.push(`${record.technicianName}: Worked days (${record.workedDays}) exceed working days (${record.workingDays})`);
        }
        
        if (!record.technicianName || !record.technicianName.trim()) errors.push(`Missing technician name for record ${record.id}`);
        if (!record.country || !record.country.trim()) errors.push(`Missing country for ${record.technicianName || 'unknown'}`);
        if (!record.currency || !record.currency.trim()) errors.push(`Missing currency for ${record.technicianName || 'unknown'}`);
    });
...
function validateSingleRow(id) {
    ...
    if (record.workingDays > 0 && record.workedDays > record.workingDays) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Check if dedicated tab is active
    const dedicatedTab = document.getElementById('dedicated-tab-content');
    if (dedicatedTab && dedicatedTab.classList.contains('active')) {
...
if (typeof window !== 'undefined') {
    ...
    window.filterDedicatedData = filterDedicatedData;
    window.recalculateDedicatedData = recalculateDedicatedData;
    window.exportDedicatedData = exportDedicatedData;
    window.validateDedicatedData = validateDedicatedData;
}
